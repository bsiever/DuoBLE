 ...
 */
void sharedCallback(BLERecipient recipient, BLECharacteristicCallbackReason reason) {
  Serial.print("sharedCallback: ");
  if(recipient == countingCharacteristic) {
  ...
  if(recipient == readWriteCharacteristic) {
    Serial.println("Read/Write Characteristic");
    if(reason == POSTWRITE) {
       byte value[20];
...
void setup() {
  ...
  countingCharacteristic.setCallback(sharedCallback);
  ...
  readWriteCharacteristic.setCallback(sharedCallback);
